...
def stockpredict(stockName):
    data = pd.read_csv('dataset/sp500_stocks.csv')
    cl = data[data['Symbol']==stockName].Close
    path = os.getcwd()
    stock_dir = os.path.join(path, "static","stocks", stockName)
    ...
    filepathtosave = os.path.join(stock_dir, stockName + ".txt")
